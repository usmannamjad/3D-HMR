...
PREDICTIONS_PATH = Path("/home/usmannamjad/vh/datasets/outputs/nlf")
...
def get_predicted_motion(file):
    pred_path = PREDICTIONS_PATH / SUBSETS[0] / f'{file}.pkl'
    with open(pred_path, 'rb') as f:
        pred = pickle.load(f)
    pred_motion = pred['pose']
    pred_motion = [p[0, :].unsqueeze(0) for p in pred_motion]
    pred_motion = torch.cat(pred_motion, dim=0)
    pred_motion = pred_motion[:, :3+63].reshape(-1, 22, 3)     # 22 body joints
...
predicted_files = get_all_file_paths(PREDICTIONS_PATH / SUBSETS[0])
...
# for predicted_file in predicted_files:
    try: 
        ...
        # jitter = compute_jitter(pred_motion)
        # print(jitter.shape)
